...
import { useGetProductsQuery, useCreateProductMutation } from '../../../api/endpoints/catalogApi';
...
export const PosTerminalPage: React.FC = () => {
  ...
  const catalogProducts = useMemo(() => {
    ...
    return combined.length > 0 ? combined : defaultFallbackProducts;
  }, [dbProductsData, customProducts]);
    ...
    <Box sx={{ display: 'flex', gap: 3, height: 'calc(100vh - 120px)' }}>
      ...
      <Dialog open={isReceiptOpen} onClose={() => setIsReceiptOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions sx={{ p: 2.5, flexDirection: 'column', gap: 1 }}>
          <Stack direction="row" spacing={1} fullWidth sx={{ width: '100%' }}>
            <Button
              fullWidth
              variant="contained"
              color="primary"
