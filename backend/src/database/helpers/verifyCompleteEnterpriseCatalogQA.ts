...
export const verifyEnterpriseCatalog = async () => {
  ...
  const mainCat = await req.post('/api/v1/catalog/categories').set('Authorization', `Bearer ${token}`).send({
  ...
  console.log(`   - Main Category Created! ID: ${mainCat.body?.data?.id}, Status: ${mainCat.status}`);

  const subCat = await req.post('/api/v1/catalog/categories').set('Authorization', `Bearer ${token}`).send({
    name: 'Smartphones ' + Date.now(),
    parentId: mainCat.body.data.id,
    description: 'Android & iOS flagship smartphones',
  });
  console.log(`   - Sub-Category Created! ID: ${subCat.body?.data?.id}, Parent ID: ${mainCat.body?.data?.id}`);
  ...
  const brandRes = await req.post('/api/v1/catalog/brands').set('Authorization', `Bearer ${token}`).send({
    name: 'Apple ' + Date.now(),
  ...
  const colRes = await req.post('/api/v1/catalog/collections').set('Authorization', `Bearer ${token}`).send({
  ...
  const attrRes = await req.post('/api/v1/catalog/attributes').set('Authorization', `Bearer ${token}`).send({
  ...
  const tagRes = await req.post('/api/v1/catalog/tags').set('Authorization', `Bearer ${token}`).send({
  ...
  const prodRes = await req.post('/api/v1/products').set('Authorization', `Bearer ${token}`).send({
  ...
  console.log(`   - Product Created! ID: ${prodRes.body?.data?.id}, Status: "${prodRes.body?.data?.status}"`);
  ...
  console.log(`   - Dynamic Filters Engine GET /api/v1/catalog/filters Status: ${filtersRes.status}`);
