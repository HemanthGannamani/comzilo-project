...
export class PayoutProviderFactory {
  public static getProvider(): IPayoutProvider {
    ...
    const hasLiveKeys = !!(process.env.RAZORPAY_PAYOUT_KEY_ID && process.env.RAZORPAY_PAYOUT_KEY_SECRET);

    if (payoutMode === 'live' && hasLiveKeys) {
      try {
        return new RazorpayPayoutProvider();
      } catch (err) {
        console.warn('[PayoutProviderFactory] Failed to initialize RazorpayPayoutProvider, falling back to MockPayoutProvider:', err);
