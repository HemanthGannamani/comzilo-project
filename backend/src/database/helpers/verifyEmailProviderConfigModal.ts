...
export const verifyEmailProviderSaving = async () => {
  ...
  const smtpRes = await req.post('/api/v1/marketing/email-providers').set('Authorization', `Bearer ${token}`).send({
    providerId: 'smtp',
  ...
  const sesRes = await req.post('/api/v1/marketing/email-providers').set('Authorization', `Bearer ${token}`).send({
    providerId: 'ses',
  ...
  const dbRows: any[] = await sequelize.query('SELECT * FROM marketing_email_providers', { type: QueryTypes.SELECT });
  ...
  const getRes = await req.get('/api/v1/marketing/email-providers').set('Authorization', `Bearer ${token}`);
