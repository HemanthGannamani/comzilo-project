...
export class VariantInventoryService {
  ...
  public async allocateWarehouse(tenantId: number | null, storeId: number | null, data: any, context?: any) {
    ...
    const inventory = await VariantInventory.create({
      ...
      status: currentStock > lowThreshold ? 'in_stock' : currentStock > 0 ? 'low_stock' : 'out_of_stock',
  ...
  public async reserveStock(tenantId: number | null, variantId: number, warehouseId: number, qty: number) {
  ...
  public async releaseReservedStock(tenantId: number | null, variantId: number, warehouseId: number, qty: number) {
    const inventory = await VariantInventory.findOne({ where: { variantId, warehouseId } });
    if (!inventory) throw new NotFoundError('Variant inventory record not found');

    const currentReserved = inventory.reservedStock || 0;
    if (qty > currentReserved) {
      throw new ValidationError(`Cannot release ${qty} reserved units. Current reserved is ${currentReserved}.`);
    }
  ...
  public async transferStock(tenantId: number | null, data: any, context?: any) {
    ...
    if (!variantId || !fromWarehouseId || !toWarehouseId || !quantity || quantity <= 0) {
      throw new ValidationError('Valid Variant ID, From Warehouse, To Warehouse, and Positive Quantity are required');
    }
    ...
    for (const item of updates) {
      if (item.variantId && item.warehouseId) {
        if (item.lowStockThreshold !== undefined) {
          const inv = await VariantInventory.findOne({ where: { variantId: item.variantId, warehouseId: item.warehouseId } });
          if (inv) {
