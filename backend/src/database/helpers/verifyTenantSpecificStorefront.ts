...
export const testTenantSpecificStorefront = async () => {
  ...
  console.log(`GET /api/v1/products Status: ${res1.status}, Count: ${res1.body?.data?.length || 0}`);
  ...
  console.log(`GET /api/v1/products?tenant_id=20&store_id=18 Status: ${res2.status}, Count: ${res2.body?.data?.length || 0}`);
  if (res2.body?.data?.length > 0) {
    console.log(`✅ Returned ${res2.body.data.length} product(s) belonging specifically to tenant 20 / store 18!`);
    console.log(`   - Sample Product: "${res2.body.data[0].name}" (Tenant ID: ${res2.body.data[0].tenantId}, Store ID: ${res2.body.data[0].storeId})`);
  }
  ...
  console.log(`GET /api/v1/products?store=satishstore Status: ${res3.status}, Count: ${res3.body?.data?.length || 0}`);
