...
export const verifyMarketingModule = async () => {
  ...
  const dashRes = await req.get('/api/v1/marketing/dashboard').set('Authorization', `Bearer ${token}`);
  ...
  console.log(`✅ Dashboard KPIs Verified! Emails Sent: ${dashRes.body.data.kpis.emailsSent}, Revenue: ₹${dashRes.body.data.kpis.revenueGenerated}`);

  // STEP 3: Email Providers
  console.log('\n[3] Fetch Configured Email Providers (GET /api/v1/marketing/email-providers)...');
  const provRes = await req.get('/api/v1/marketing/email-providers').set('Authorization', `Bearer ${token}`);
  ...
  const tplRes = await req.post('/api/v1/marketing/email-templates').set('Authorization', `Bearer ${token}`).send({
  ...
  const cmpRes = await req.post('/api/v1/marketing/campaigns').set('Authorization', `Bearer ${token}`).send({
  ...
  const cpnRes = await req.post('/api/v1/marketing/coupons').set('Authorization', `Bearer ${token}`).send({
  ...
  console.log(`✅ Coupon Created! ID: ${cpnRes.body?.data?.id}, Code: "${cpnRes.body?.data?.code}"`);

  // STEP 7: Abandoned Carts Tracker
  console.log('\n[7] Fetch Abandoned Carts Tracker (GET /api/v1/marketing/abandoned-carts)...');
  const cartsRes = await req.get('/api/v1/marketing/abandoned-carts').set('Authorization', `Bearer ${token}`);
  console.log(`GET /api/v1/marketing/abandoned-carts Status: ${cartsRes.status}`);
  console.log(`✅ Abandoned Carts Tracker Verified! Carts tracked: ${cartsRes.body?.data?.length || 0}`);

  // STEP 8: Customer Segments Creation & Fetch
  console.log('\n[8] Create Customer Segment "VIP High Value" (POST /api/v1/marketing/segments)...');
  const segRes = await req.post('/api/v1/marketing/segments').set('Authorization', `Bearer ${token}`).send({
  ...
  const ruleRes = await req.post('/api/v1/marketing/automation-rules').set('Authorization', `Bearer ${token}`).send({
